...
//                serialize16(inclination.raw[i]);
         ...
         //   serialize16(althold_throttle);
            break;
        case MSP_ALTITUDE:
            headSerialReply(6);
#if defined(BARO)
        //    serialize32(GPS_numSat*100);
          //
        ...
        case MSP_ANALOG:
            headSerialReply(7);
            serialize8((uint8_t) baroGroundTemperature);
            serialize16((uint16_t)posX);
            serialize16((uint16_t)posY);
            serialize16((uint16_t)posZ);
