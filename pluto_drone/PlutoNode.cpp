...
void *serviceFunction(void *threadid)
{
  while (1) 
  {
      if (serviceClient.call(service))
      {
       ...
       service.request.posx=posx;
       service.request.posy=posy;
       service.request.posz=posz;
       service.request.temp=temp;
      }
  }
 pthread_exit(NULL);
}
void Callback(const plutodrone::PlutoMsg::ConstPtr& msg)
{
 ...
 userRC[6] = msg->rcAUX3;
 userRC[7] = msg->rcAUX4;
 cout<<"Roll"<<userRC[0]<<" Pitch"<<userRC[1]<<" Throttle"<<userRC[2]<<" Yaw"<<userRC[3]<<" AUX1"<<userRC[4]<<" AUX2"<<userRC[5]<<" AUX3"<<userRC[6]<<" AUX4"<<userRC[7];
 cout<<endl<<"######"<<endl;
}
